<script lang="ts">
	import type { QuickStartRequest } from "$lib/bindings-types"
    ...
    let recent_projects = [
        {
            name: "Ducky delusions",
            path: "C:/Program files/Epic games/something/probably/DuckyMod",
            time: new Date(),
            version: "1.01"
        },
        {
            name: "Rocco's return",
            path: "C:/Program files/Steam/steamapps/HITMAN/SMF/Mods/Proj3",
            time: new Date(),
            version: "2.01"
        },
        {
            name: "Now remember folks, the secret of endurance racing...",
            path: "C:/User/Rocco/Windows/Gaming/Hitman/mod",
            time: new Date(),
            version: "1.21"
        },
    ]
	...
	export async function handleRequest(request: QuickStartRequest) {
		console.log(`Project hub ${id} handling request`, request)
	}
</script>
        ...
        <div class="flex-1">
            <div class="flex flex-col justify-between">
                ...
                <div class="flex flex-col">
                    ...
                    <ClickableTile class="mt-2" on:click={()=>{
                        console.log(`I would need to open ${project.name} now`)
                    }}>
                            ...
                            <div>
                                ...
                                <div class="flex">
                                    <div class="flex-grow">
                                        ...
                                        <p class="pt-1 text-xs">{project.time.toLocaleString()}</p>
                                    </div>
                                    <OverflowMenu flipped on:click={(e)=>{
                                        e.stopPropagation();
                                        console.log("overflow menu pressed")
                                    }}>
                                        <OverflowMenuItem text="Show in explorer" on:click={(e)=>{
                                            e.stopPropagation();
                                            console.log("Show in explorer pressed")
                                        }}/>
                                        <OverflowMenuItem danger text="Remove recent" on:click={(e)=>{
                                            e.stopPropagation();
                                            console.log("Remove recent pressed")
                                        }}/>
                                    </OverflowMenu>
                                </div>
